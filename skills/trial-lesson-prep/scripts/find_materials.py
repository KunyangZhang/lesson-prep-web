...
def build_result(status: str, root: Path, stage: str, grade: str, keywords: list[str], candidates: list[dict]) -> dict:
    return {
        "status": status,
        "root": str(root),
        "query": {
...
def main() -> int:
    ...
    for item in ranked:
        item["lastWriteTime"] = __import__("datetime").datetime.fromtimestamp(item["lastWriteTime"]).isoformat(timespec="seconds")
